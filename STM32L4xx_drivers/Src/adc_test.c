...
void InitAdcGpio(void)
{
	GPIO_Handle_t tADCGpio = {0};

	tADCGpio.pGPIOx = GPIOC;
	tADCGpio.GpioPinConfig.GPIO_PinMode = eGPIO_AnalogMode;
	tADCGpio.GpioPinConfig.GPIO_PinOpType = eGPIO_OpPushPull;
	tADCGpio.GpioPinConfig.GPIO_PinSpeed = eGPIO_VeryHighSpeed;
	tADCGpio.GpioPinConfig.GPIO_PinPuPdCtrl = eGPIO_NoPuPd;

	GPIO_PeriClkCtrl(GPIOC, ENABLE);

	/* Configure TX pin. */
	tADCGpio.GpioPinConfig.GPIO_PinNumber = eGPIO_PIN_0;
	GPIO_Init(&tADCGpio);
...
void InitAdc3(void)
{
	...
	tAdc3.pADC = ADC3;
	tAdc3.ADCConfig.ucAdcChannelId = eAdcChannelId1;
	tAdc3.ADCConfig.ucAdcConvMode = eAdcSingleConvMode;
	tAdc3.ADCConfig.ucAdcDataAllingment = eAdcDataAlignRight;
	tAdc3.ADCConfig.ucAdcReslnCnfg = eAdcResln12bit;
	tAdc3.ADCConfig.ucAdcSamplingTime = eAdcSamplingTime12_5Clk;
...
void TestAdc(void)
{
	...
	/* Read ADC value. */
	uhAdcVal = AdcReadChannel(ADC3, eAdcChannelId1);

	/* Copy result to TX buffer. */
	uhTxSize = sprintf((char*)ucTxBuffer, "The ADC value is : %d\r\n",uhAdcVal);

	/* Send to UART. */
	USART_TxData(USART2, ucTxBuffer,uhTxSize);
}
